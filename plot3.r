...
png("plot3.png",bg="transparent") #default size of png is 480x480 so there's no need to change them
#png("plot3.png") #if having troubles with the transparency, uncomment this and comment the other

plot(strptime(paste(dt[[1]],dt[[2]]),"%d/%m/%Y %H:%M:%S"),dt[[3]],type="l",xlab="",ylab="Energy sub metering")
lines(strptime(paste(dt[[1]],dt[[2]]),"%d/%m/%Y %H:%M:%S"),dt[[4]],col="red")
lines(strptime(paste(dt[[1]],dt[[2]]),"%d/%m/%Y %H:%M:%S"),dt[[5]],col="blue")
